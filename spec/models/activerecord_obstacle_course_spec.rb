...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    order = Order.last
    names = order.items.all.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '23. returns all orders which include item_4' do
    ...
    orders = Order.select("orders.*").joins(:items).where("items.id = ?", 4)
    # -----------------------------------------------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    # ------------------------------------------------------------
    ordered_items_names = Item.select("items.id, items.name").joins(:order_items).order("items.id").distinct.pluck(:name)
    # ------------------ ActiveRecord Solution ----------------------
    # Solution goes here
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.select("users.name, count(items.id) AS total_item_count").joins(:items).group(:name).order(:name).reverse_order
    # ---------------------------------------------------------------
